import React, { useState } from 'react';
import { 
  MapPin, Clock, Navigation, Coffee, ShoppingBag, 
  Sparkles, Train, Utensils, Award, CheckCircle2, 
  Copy, Info, ChevronRight, Bookmark, Search, Filter
} from 'lucide-react';

export default function App() {
  const [activeDay, setActiveDay] = useState(0); // 0 = 全部, 1-5 = 每日
  const [selectedTag, setSelectedTag] = useState('ALL');
  const [copiedId, setCopiedId] = useState<string | null>(null);
  const [showTips, setShowTips] = useState(false);

  // 複製地址輔助函式 (相容 iFrame 環境)
  const handleCopy = (text: string, id: string) => {
    const el = document.createElement('textarea');
    el.value = text;
    document.body.appendChild(el);
    el.select();
    document.execCommand('copy');
    document.body.removeChild(el);

    setCopiedId(id);
    setTimeout(() => setCopiedId(null), 2000);
  };

  // 智慧產生 Google Maps 搜尋關鍵字
  const getMapQuery = (title: string, address: string) => {
    const primaryTitle = title.split(' 或 ')[0].split(' / ')[0].replace(/（.*?）|\(.*?\)/g, '').trim();
    const cleanAddress = address.split(' / ')[0].split(' ｜ ')[0].replace(/（.*?）|\(.*?\)/g, '').trim();
    return `${primaryTitle} ${cleanAddress}`;
  };

  const tagDefinitions = [
    { id: 'ALL', label: '全部項目', icon: Sparkles, color: 'bg-slate-800 text-white' },
    { id: 'TOY', label: '陀螺 / 吉伊卡哇 / Capcom', icon: ShoppingBag, color: 'bg-purple-600 text-white' },
    { id: 'DRINK', label: '精品咖啡 & 絲滑奶茶', icon: Coffee, color: 'bg-amber-600 text-white' },
    { id: 'FOOD', label: '美式漢堡 & 越南河粉', icon: Utensils, color: 'bg-emerald-600 text-white' },
    { id: 'SCENIC', label: '經典景點 & 在地名店', icon: Award, color: 'bg-rose-600 text-white' }
  ];

  const itineraryData = [
    // --- DAY 1 ---
    {
      id: 'd1-1',
      day: 1,
      dayTitle: 'Day 1 (9/24 週四)',
      timeSlot: '12:00 - 12:30',
      category: 'SCENIC',
      highlightType: 'scenic',
      title: '抵達油麻地住宿 ＆ Check-in 寄放行李',
      subtitle: '市區中心安頓行囊 ＆ 輕裝上陣開啟香港自由行',
      address: '香港油麻地彌敦道住宿飯店',
      hours: '飯店全日接待（行李可免費寄存禮賓部）',
      arrivalTime: '約 12:00 抵達油麻地飯店辦理手續',
      transit: {
        line: '機場快線 ➔ 的士 / 城巴 A21',
        station: '油麻地站 / 九龍站',
        exit: '油麻地站各出口即達彌敦道',
        walk: '香港機場搭機場快線至九龍站搭的士（約 8 分鐘，HK$40-50）或搭城巴 A21 直達油麻地'
      },
      tagText: '🧳 飯店 Check-in & 行李寄存',
      tagColor: 'bg-blue-100 text-blue-800 border-blue-300 font-bold',
      notes: '12:00 順利抵達油麻地飯店辦妥入住或將大件行李寄存在櫃檯，換上隨身隨行小包，以最輕盈的狀態正式開啟香港 5 天之旅！'
    },
    {
      id: 'd1-2',
      day: 1,
      dayTitle: 'Day 1 (9/24 週四)',
      timeSlot: '12:45 - 14:00',
      category: 'FOOD',
      highlightType: 'pho',
      title: 'Brass Spoon（米其林必比登越式河粉）',
      subtitle: '牛骨長時熬煮極清甜鮮牛肉河粉 ＆ 酥脆油條泡湯絕配',
      address: '灣仔月街 1 號 B 地舖 / 中環威靈頓街 50 號',
      hours: '12:00 - 19:00',
      arrivalTime: '建議 12:45 抵達享用午餐',
      transit: {
        line: '港鐵 荃灣線 ➔ 港島線',
        station: '金鐘站 或 灣仔站',
        exit: '金鐘站 F 出口 / 灣仔站 A3 出口',
        walk: '從油麻地搭荃灣線僅 4 站（約 11 分鐘）直達金鐘，出站步行約 6 分鐘即達'
      },
      tagText: '🍜 米其林必比登鮮甜越式河粉',
      tagColor: 'bg-emerald-100 text-emerald-800 border-emerald-300 font-bold',
      notes: '全香港評價最高的 Pho 之一！以純牛骨長時精熬出的湯頭清甜回甘、香醇不膩，鮮生牛肉片嫩滑多汁，加點酥脆油條浸泡湯汁入口更是絕配。💡【方便度極致建議】：若剛下飛機稍感疲憊想免去過海交通，亦可直接在油麻地飯店周邊（如富華冰室、興記煲仔飯、生記粥品）或直接前往朗豪坊美食街就近用餐，隨心安排最彈性！'
    },
    {
      id: 'd1-3',
      day: 1,
      dayTitle: 'Day 1 (9/24 週四)',
      timeSlot: '14:15 - 15:15',
      category: 'SCENIC',
      highlightType: 'scenic',
      title: '香港百年叮叮車體驗（灣仔 ➔ 銅鑼灣）',
      subtitle: '雙層露天復古路面電車慢速街景巡禮',
      address: '灣仔莊士敦道沿線電車站（東行方向）',
      hours: '06:00 - 00:00',
      arrivalTime: '隨上隨搭（八達通刷卡只需 HK$3.0）',
      transit: {
        line: '香港電車（叮叮車）',
        station: '柯布連道 / 巴路士街 電車站',
        exit: '後門上車，前門下車刷卡',
        walk: '吃完 Brass Spoon 漫步至莊士敦道電車站上車'
      },
      tagText: '🚋 港島百年雙層電車',
      tagColor: 'bg-blue-100 text-blue-800 border-blue-300',
      notes: '一定要坐上層最前排！吹著微風俯瞰香港招牌林立的街道，體驗老香港情懷最惬意方式。搭至銅鑼灣後，搭乘港鐵港島線至金鐘轉荃灣線回旺角（全程僅約 14 分鐘）。'
    },
    {
      id: 'd1-4',
      day: 1,
      dayTitle: 'Day 1 (9/24 週四)',
      timeSlot: '15:30 - 17:30',
      category: 'TOY',
      highlightType: 'toy',
      title: '朗豪坊 Langham Place（旺角）',
      subtitle: '高層動漫扭蛋玩具大本營 ＆ 吉伊卡哇週邊天堂',
      address: '旺角亞皆老街 8 號（重點逛 10 樓 – 11 樓）',
      hours: '11:00 - 22:00',
      arrivalTime: '建議 15:30 抵達悠閒逛街',
      transit: {
        line: '港鐵 荃灣線 / 觀塘線',
        station: '旺角站',
        exit: 'C3 / E1 出口',
        walk: '旺角站 C3 地下通道直達朗豪坊 B2 樓層'
      },
      tagText: '✨ Chiikawa 吉伊卡哇 & 日系盒玩扭蛋',
      tagColor: 'bg-purple-100 text-purple-800 border-purple-300 font-bold',
      notes: '10-11 樓聚集了大型玩具店、扭蛋區與日系動漫專賣店，吉伊卡哇（Chiikawa）玩偶、文具、限定盲盒品項最齊全！'
    },
    {
      id: 'd1-5',
      day: 1,
      dayTitle: 'Day 1 (9/24 週四)',
      timeSlot: '17:45 - 20:00',
      category: 'TOY',
      highlightType: 'toy',
      title: '現時點 In’s Point ＆ 信和中心（油麻地／旺角）',
      subtitle: '香港戰鬥陀螺聖地 ＆ Capcom/ACG 模型聚集地',
      address: '現時點：彌敦道 530-538 號 2 樓 ｜ 信和中心：彌敦道 582 號',
      hours: '16:00 - 21:30（店家約 16:30 開齊）',
      arrivalTime: '強烈建議 17:45 抵達（店家最齊全時段）',
      transit: {
        line: '港鐵 荃灣線 / 觀塘線',
        station: '油麻地站',
        exit: 'A2 出口',
        walk: '出站向右轉直走 1 分鐘即達現時點；往旺角方向直走 3 分鐘達信和中心'
      },
      tagText: '🔥 戰鬥陀螺限定抽包/散件 & Capcom模型',
      tagColor: 'bg-amber-100 text-amber-900 border-amber-400 font-bold',
      notes: '【全港陀螺核心地標】現時點 2 樓有極多專門店，Beyblade X 稀有抽包、單售配件/零件、特惠整盒應有盡有；信和中心則專攻 Capcom（魔物獵人、快打旋風、惡靈古堡）遊戲周邊與手辦。買完戰利品步行 2 分鐘即可先送回油麻地飯店！'
    },
    {
      id: 'd1-6',
      day: 1,
      dayTitle: 'Day 1 (9/24 週四)',
      timeSlot: '20:15 - 21:45',
      category: 'FOOD',
      highlightType: 'pho',
      title: '一點心 One Dim Sum ❤️‍🔥（米其林平價點心天花板）',
      subtitle: '招牌晶瑩鮮蝦餃、炸蒜魷魚鬚、叉燒包與香煎蘿蔔糕',
      address: '太子運動場道 15 號京華大廈地舖',
      hours: '10:00 - 00:00（營業至午夜）',
      arrivalTime: '建議 20:15 逛完現時點/信和中心後前往',
      transit: {
        line: '港鐵 荃灣線 / 觀塘線',
        station: '太子站',
        exit: 'A 出口',
        walk: '出站步行 2 分鐘（從旺角信和中心搭 1 站港鐵或沿彌敦道步行 10 分鐘）'
      },
      tagText: '🥟 米其林必比登星級點心宵夜',
      tagColor: 'bg-rose-100 text-rose-800 border-rose-300 font-bold',
      notes: '【Threads 清單大推 ❤️‍🔥】全港評價最高且超平價的米其林推薦點心名店！完美解決原大安茶氷廳 19:00 打烊的問題。買完陀螺與動漫周邊後，來籠熱騰騰爆汁鮮蝦餃、蒜香濃郁炸魷魚鬚與流沙包，極致滿足！'
    },

    // --- DAY 2 ---
    {
      id: 'd2-1',
      day: 2,
      dayTitle: 'Day 2 (9/25 週五·深圳一日遊)',
      timeSlot: '09:30 - 11:00',
      category: 'SCENIC',
      highlightType: 'scenic',
      title: '香港西九龍高鐵 ➔ 深圳福田站',
      subtitle: '一地兩檢快速通關 ＆ 14 分鐘極速直達深圳 CBD',
      address: '香港西九龍站 ➔ 深圳福田站',
      hours: '高鐵班次極為密集（約 10-15 分鐘一班）',
      arrivalTime: '09:30 油麻地出發，搭約 10:15 高鐵班次',
      transit: {
        line: '香港市區紅的 ➔ 廣深港高鐵',
        station: '香港西九龍站 ➔ 福田站',
        exit: '福田站出站直通連城新天地地下商業街',
        walk: '油麻地飯店搭的士 6 分鐘直達西九龍站（約 HK$30）；高鐵 14 分鐘直達福田'
      },
      tagText: '🚄 廣深港高鐵 14 分鐘極速直達',
      tagColor: 'bg-blue-100 text-blue-800 border-blue-300 font-bold',
      notes: '憑台胞證實名制直接刷進出閘門，西九龍站一次辦完香港出境與大陸入境（一地兩檢）。福田與西九龍支援「靈活行」，當天可免費改簽 3 次。提前綁定好微信支付或支付寶信用卡。'
    },
    {
      id: 'd2-2',
      day: 2,
      dayTitle: 'Day 2 (9/25 週五·深圳一日遊)',
      timeSlot: '11:15 - 12:45',
      category: 'FOOD',
      highlightType: 'pho',
      title: '八合里牛肉火鍋（卓悅中心店 / 購物公園店）',
      subtitle: '正宗潮汕鮮切溫體牛肉 ＆ 招牌爆汁手打牛筋丸',
      address: '深圳市福田區福華一路卓悅中心 B1 層 / 民田路購物公園北園 2 樓',
      hours: '11:00 - 22:00',
      arrivalTime: '建議 11:15 抵達入座第一輪免排隊',
      transit: {
        line: '深圳地鐵 3 號線 / 連城新天地地下街',
        station: '購物公園站 / 崗廈站 / 福田站',
        exit: '地下街或商場出口直達',
        walk: '高鐵福田站走連城新天地地下冷氣街約 9 分鐘（或打車 12 元起步價）'
      },
      tagText: '🍲 潮汕現切嫩牛肉 & 爆汁手打牛丸',
      tagColor: 'bg-rose-100 text-rose-800 border-rose-300 font-bold',
      notes: '每日新鮮溫體牛配送！必點「吊龍」（肉質極滑嫩鮮甜）、「匙柄」（微脆帶筋爽口）、「手打生牛筋丸」（爆漿彈牙）。清湯底涮 8-10 秒微粉紅即起鍋，沾自調沙茶醬配炸蒜酥是人間美味！'
    },
    {
      id: 'd2-3',
      day: 2,
      dayTitle: 'Day 2 (9/25 週五·深圳一日遊)',
      timeSlot: '13:00 - 14:15',
      category: 'TOY',
      highlightType: 'toy',
      title: '福田星河 COCO Park（潮流動漫潮玩）',
      subtitle: '巨型 POP MART 旗艦店 ＆ TOP TOY 潮玩盒玩大本營',
      address: '深圳市福田區福華三路 268 號',
      hours: '10:00 - 22:00',
      arrivalTime: '建議 13:00 吃飽後悠閒採購',
      transit: {
        line: '深圳地鐵 1 號線 / 3 號線',
        station: '購物公園站',
        exit: 'C / G 出口直通商場',
        walk: '八合里吃完漫步 2 分鐘即達'
      },
      tagText: '✨ 泡泡瑪特旗艦 & TOP TOY 潮流盲盒',
      tagColor: 'bg-purple-100 text-purple-800 border-purple-300 font-bold',
      notes: '大型露天公園式購物商場！POP MART 旗艦店有極震撼的巨型公仔展示與最新盲盒；TOP TOY 匯集各類動漫 IP 與積木雕像。'
    },
    {
      id: 'd2-4',
      day: 2,
      dayTitle: 'Day 2 (9/25 週五·深圳一日遊)',
      timeSlot: '14:15 - 15:00',
      category: 'DRINK',
      highlightType: 'tea',
      title: '霸王茶姬 ＆ 瑞幸咖啡（COCO Park 店）',
      subtitle: '原葉鮮奶茶天花板「伯牙絕弦」＆ 國民爆款「生椰拿鐵」',
      address: '深圳市福田區福華三路 268 號福田星河 COCO Park B1 層（兩店皆在商場 B1）',
      hours: '10:00 - 22:00',
      arrivalTime: '建議 13:45 逛潮玩時先用微信/支付寶小程式「遠端手機下單」',
      transit: {
        line: '商場內部 B1 層步行直達',
        station: '購物公園站',
        exit: 'C / G 出口直通 COCO Park B1',
        walk: '從 POP MART / TOP TOY 搭手扶梯至 B1 即可直接取餐'
      },
      tagText: '🧋 霸王茶姬伯牙絕弦 & 瑞幸生椰拿鐵',
      tagColor: 'bg-amber-100 text-amber-900 border-amber-300 font-bold',
      notes: '【COCO Park 兩大必喝手搖·免排隊點單密技】：\n1. 霸王茶姬（B1 層）：首推神級王牌「伯牙絕弦」（茉莉雪芽茶底＋優質牛乳，花香茶韻清甜絲滑，建議微糖/少冰不膩口）或「萬里木蘭」（厚實紅茶香）。\n2. 瑞幸咖啡（B1 層）：必喝經典「生椰拿鐵」（冷萃厚生椰乳融合現磨濃縮咖啡，香濃滑順微甜）或夏日首選「橙 C 美式」。\n💡【免排隊攻略】：在 1 樓逛 POP MART 時，直接打開微信搜尋「霸王茶姬」與「瑞幸咖啡」小程式點單支付，做好後手機會推播通知，逛完下樓直接「憑取餐碼」秒拿，完全免排現場人潮！邊喝邊前往旁邊的洗頭 SPA 極度享受！'
    },
    {
      id: 'd2-5',
      day: 2,
      dayTitle: 'Day 2 (9/25 週五·深圳一日遊)',
      timeSlot: '15:15 - 16:30',
      category: 'SCENIC',
      highlightType: 'tea',
      title: '沉浸式水循環洗頭 SPA ＆ 肩頸指腹放鬆',
      subtitle: '溫水光環水療瀑布、深層淨化頭皮（無痛無癢·舒適助眠）',
      address: '福田 CBD 商圈 / 皇庭廣場 / 連城新天地洗頭護理館',
      hours: '10:00 - 22:00',
      arrivalTime: '下午逛累時段享受（大眾點評團購約 68-98 RMB，500 台幣有找）',
      transit: {
        line: '福田 CBD 地下商業步行軸',
        station: '購物公園站 / 會展中心站',
        exit: '地下街直通各商場',
        walk: '從 COCO Park 步行 3-5 分鐘即達'
      },
      tagText: '💆 溫水水療循環洗頭 (預算500台幣內)',
      tagColor: 'bg-emerald-100 text-emerald-800 border-emerald-300 font-bold',
      notes: '【不痛不癢放心體驗】避開會發癢的羽毛採耳與刺痛震叉！此項目專注溫熱水循環水流沖淋＋指腹穴位溫柔揉捏，徹底放鬆頭頸。進店暗號：「洗頭請全程用『指腹』輕揉，不要用指甲抓；力道放輕，我想放鬆休息」。'
    },
    {
      id: 'd2-6',
      day: 2,
      dayTitle: 'Day 2 (9/25 週五·深圳一日遊)',
      timeSlot: '16:45 - 18:30',
      category: 'SCENIC',
      highlightType: 'scenic',
      title: '深圳卓悅中心 One Avenue',
      subtitle: '800 米奢華中央步行大街 ＆ 國際潮流生活街區',
      address: '深圳市福田區福華一路（崗廈站旁）',
      hours: '10:00 - 22:00',
      arrivalTime: '建議 16:45 享受午後微風街景',
      transit: {
        line: '深圳地鐵 1 號線 / 10 號線',
        station: '崗廈站 / 會展中心站',
        exit: '崗廈站 B 出口直通商場',
        walk: '從皇庭廣場步行約 8 分鐘（或搭 1 站地鐵 / 打車約 12 元）'
      },
      tagText: '🏙️ 深圳旗艦中央大街 & 網紅露天街區',
      tagColor: 'bg-amber-100 text-amber-900 border-amber-300',
      notes: '深圳最大型戶外街區購物中心，擁有長達 800 公尺的林蔭中央大街，兩側匯聚潮流旗艦、文創書店與咖啡座，現代都市感極強，拍照散步非常愜意。'
    },
    {
      id: 'd2-7',
      day: 2,
      dayTitle: 'Day 2 (9/25 週五·深圳一日遊)',
      timeSlot: '18:45 - 20:30',
      category: 'FOOD',
      highlightType: 'burger',
      title: '盒馬鮮生（皇庭廣場店）＆ 滿載返港',
      subtitle: '現撈海鮮現烹、平價當季精品水果 ＆ 特色烘焙伴手禮',
      address: '深圳市福田區福華三路 118 號皇庭廣場 B1 層',
      hours: '09:00 - 22:00',
      arrivalTime: '建議 18:45 前往採買晚餐海鮮與水果',
      transit: {
        line: '深圳地鐵 1 號線 / 4 號線 ➔ 高鐵',
        station: '會展中心站 C 出口 / 福田高鐵站',
        exit: '會展中心 C 出口直達皇庭廣場 B1',
        walk: '逛完走連城新天地地下冷氣街 10 分鐘直達高鐵福田站'
      },
      tagText: '🦞 盒馬鮮生海鮮水果 & 高鐵直達返港',
      tagColor: 'bg-indigo-100 text-indigo-800 border-indigo-300 font-bold',
      notes: '必買新鮮當季水蜜桃、泰國椰青與烘焙甜點；亦可在海鮮區點現蒸羅氏蝦、生蠔當晚餐！採買完沿地下街步行至福田站，搭 14 分鐘高鐵直奔香港西九龍站，再搭 5 分鐘的士輕鬆回到油麻地飯店！'
    },


    // --- DAY 3 ---
    {
      id: 'd3-1',
      day: 3,
      dayTitle: 'Day 3 (9/26 週六)',
      timeSlot: '09:30 - 11:30',
      category: 'SCENIC',
      highlightType: 'scenic',
      title: '大館 Tai Kwun ＆ 中環半山手扶梯',
      subtitle: '前中區警署百年紅磚古蹟群 ＆ 世界最長戶外有蓋手扶梯漫步',
      address: '香港中環荷李活道 10 號（大館） ｜ 中環中街至半山（手扶梯）',
      hours: '08:00 - 23:00（大館建築群免費參觀）',
      arrivalTime: '建議 09:30 - 10:00 抵達（早晨遊客少、光影極美最好拍）',
      transit: {
        line: '港鐵 荃灣線 (紅線) 直達',
        station: '中環站',
        exit: 'D2 / C 出口',
        walk: '油麻地搭港鐵 12 分鐘直達中環站，出站沿德己立街轉皇后大道中，搭半山手扶梯至荷李活道（直通大館檢閱廣場）'
      },
      tagText: '⭐ 百年紅磚警署古蹟群 & 半山手扶梯',
      tagColor: 'bg-rose-100 text-rose-800 border-rose-300 font-bold',
      notes: '【中環早晨經典文化景點】16 座維多利亞及愛德華風格紅磚歷史建築群，前身為香港警署、裁判司署與域多利監獄。搭乘半山手扶梯沿途可欣賞經典嘉咸街塗鴉壁畫、骨董街，漫步進入大館檢閱廣場與營房大樓，晨光下紅磚拍照極具張力且免費開放！'
    },
    {
      id: 'd3-2',
      day: 3,
      dayTitle: 'Day 3 (9/26 週六)',
      timeSlot: '11:45 - 13:15',
      category: 'FOOD',
      highlightType: 'pho',
      title: '蓮香樓 Lin Heung Lau（百年老字號傳統飲茶）',
      subtitle: '經典阿姐手推點心車、天地人蓋碗普洱茶 ＆ 懷舊粵式名店',
      address: '香港上環德輔道中 249-253 號東寧大廈地下 9-10 號舖及 1-2 樓（近港鐵上環站 B / A1 出口；原中環威靈頓街舊址因重建搬遷至此，步行僅約 6 分鐘）',
      hours: '06:00 - 00:00（全日營業）',
      arrivalTime: '建議 11:45 抵達入座午茶時段（避開大排長龍）',
      transit: {
        line: '步行自大館順路漫步 ➔ 或上環站',
        station: '上環站 / 中環站',
        exit: '上環站 B / A1 出口',
        walk: '從大館沿荷李活道往上環方向轉急庇利街/禧利街下坡，漫步約 6-8 分鐘即達德輔道中東寧大廈'
      },
      tagText: '🥟 懷舊手推點心車 & 招牌豬潤燒賣',
      tagColor: 'bg-rose-100 text-rose-800 border-rose-300 font-bold',
      notes: '【順路午餐·體驗百年推車飲茶文化】全港極少數仍保留阿姐推著點心車巡迴、食客手持點心卡搶剛出爐點心的經典老字號！必拿「豬潤燒賣」（豬肝厚切鮮嫩）、「霸王鴨」、「蜜汁叉燒包」、「鮮蝦腐皮捲」與「香滑馬拉糕」，配上一盅熱水蓋碗茶，深度體驗最地道熱鬧的港式飲茶氛圍！'
    },
    {
      id: 'd3-3',
      day: 3,
      dayTitle: 'Day 3 (9/26 週六)',
      timeSlot: '13:30 - 15:30',
      category: 'DRINK',
      highlightType: 'coffee',
      title: '蘭桂坊 Lan Kwai Fong ＆ 中環精品咖啡/蛋撻午茶',
      subtitle: '經典歐陸鵝卵石斜坡酒吧街 ＆ 白天異國漫步／午後 Happy Hour',
      address: '中環德己立街、蘭桂坊及和安里一帶',
      hours: '全日開放（街區各特色餐酒館、露天酒吧約 12:00 起營業）',
      arrivalTime: '建議 13:30 蓮香樓吃飽後散步前來',
      transit: {
        line: '步行自蓮香樓 / 叮叮車體驗',
        station: '中環站',
        exit: 'D1 / D2 出口',
        walk: '沿德輔道中往中環方向步行約 8 分鐘（亦可在蓮香樓門口搭 2 站「叮叮車」至畢打街，再走 3 分鐘轉入德己立街）'
      },
      tagText: '🍸 經典歐美餐酒吧街 & 午後小酌',
      tagColor: 'bg-purple-100 text-purple-800 border-purple-300 font-bold',
      notes: '【香港最著名夜生活地標】蘭桂坊白天呈現出截然不同的悠閒歐美氛圍，L 形石板斜坡街道兩旁滿是異國小酒館與戶外露天座位，非常適合散步拍照。💡【順路午茶推薦】：蘭桂坊旁即是中環精品咖啡聚落：\n1. 咖啡控可步行 3 分鐘至 The Cupping Room（閣麟街 18 號）或 NOC Coffee Co.（嘉咸街 34 號）品嚐手沖冠軍咖啡或絲滑 Flat White。\n2. 蛋撻控必訪擺花街 5 號的 Bakehouse 外帶招牌酸種蛋撻！\n3. 若喜愛美式漢堡，擺花街 8 號的 Honbo（手打 Smash Burger）也在同一條街上！'
    },
    {
      id: 'd3-4',
      day: 3,
      dayTitle: 'Day 3 (9/26 週六)',
      timeSlot: '16:00 - 19:15',
      category: 'SCENIC',
      highlightType: 'scenic',
      title: '太平山頂纜車 ＆ 凌霄閣摩天台428',
      subtitle: '第六代綠色復古山頂纜車 ＆ 360° 戶外夜景觀景台',
      address: '香港中環花園道 33 號山頂纜車總站',
      hours: '07:30 - 23:00（摩天台開放至 23:00）',
      arrivalTime: '強烈建議 16:00 - 16:30 抵達花園道纜車站上山',
      transit: {
        line: '步行自蘭桂坊 ➔ 中環花園道',
        station: '中環站 / 金鐘站',
        exit: '中環站 J2 出口 / 金鐘站 C1 出口',
        walk: '從蘭桂坊沿德己立街轉皇后大道中，穿過遮打花園與香港公園天橋步行約 10-12 分鐘即達花園道纜車總站'
      },
      tagText: '⭐ 百萬維港黃昏與夜景',
      tagColor: 'bg-rose-100 text-rose-800 border-rose-300 font-bold',
      notes: '16:30 上山能同時看到日景、夕陽餘暉與全黑後的百萬夜景。線上先買「纜車+摩天台」二合一套票掃 QR Code 進站最省時。'
    },
    {
      id: 'd3-5',
      day: 3,
      dayTitle: 'Day 3 (9/26 週六)',
      timeSlot: '19:45 - 21:15',
      category: 'FOOD',
      highlightType: 'pho',
      title: '九記牛腩 或 一樂燒鵝（中環經典晚餐）',
      subtitle: '香港米其林必比登／星級傳統老字號',
      address: '中環歌賦街 21 號（九記） / 中環士丹利街 34-38 號（一樂）',
      hours: '12:30 - 22:30（九記週日公休）',
      arrivalTime: '建議 19:45 纜車下山後前往',
      transit: {
        line: '山頂纜車下山 ➔ 漫步中環',
        station: '中環站',
        exit: 'D2 出口',
        walk: '從花園道纜車站漫步下山至士丹利街約 10 分鐘、歌賦街約 13 分鐘'
      },
      tagText: '🍜 濃郁清湯牛腩 / 脆皮燒鵝',
      tagColor: 'bg-orange-100 text-orange-800 border-orange-300 font-bold',
      notes: '九記推薦「上湯牛腩粗麵」湯頭鮮甜回甘；一樂燒鵝外皮油亮酥脆，肉質極為軟嫩。只收現金或八達通。💡【夜晚蘭桂坊加碼】：用完晚餐後，蘭桂坊就在士丹利街上方（步行 3 分鐘），此時正是蘭桂坊越夜越熱鬧的微醺酒吧高峰，可隨意選一家戶外酒吧聽音樂小酌，享受完整的香港中環之夜！'
    },

    // --- DAY 4 ---
    {
      id: 'd4-1',
      day: 4,
      dayTitle: 'Day 4 (9/27 週日·中秋節)',
      timeSlot: '09:00 - 10:15',
      category: 'DRINK',
      highlightType: 'tea',
      title: '澳洲牛奶公司（佐敦）',
      subtitle: '全港最滑嫩炒蛋多士 ＆ 濃醇熱奶茶',
      address: '佐敦白加士街 47-49 號地下',
      hours: '07:30 - 22:00（週四公休，週日正常營業）',
      arrivalTime: '建議 09:00 前抵達排隊（翻桌極快約 10 分鐘）',
      transit: {
        line: '港鐵 荃灣線',
        station: '佐敦站',
        exit: 'C2 出口',
        walk: '出站步行 2 分鐘（從油麻地走路也只需 8 分鐘）'
      },
      tagText: '🥪 滑蛋厚多士 & 蛋白燉鮮奶',
      tagColor: 'bg-amber-100 text-amber-900 border-amber-300',
      notes: '炒蛋奶油香氣撲鼻、水嫩多汁；配一杯「熱奶茶，走甜」，享受最正宗純粹的厚茶乳香。只收現金。'
    },
    {
      id: 'd4-2',
      day: 4,
      dayTitle: 'Day 4 (9/27 週日·中秋節)',
      timeSlot: '10:45 - 13:30',
      category: 'TOY',
      highlightType: 'toy',
      title: '海港城 ＆ 玩具"反"斗城旗艦店（尖沙咀）',
      subtitle: '全港最大旗艦玩具店 ＆ 戰鬥陀螺官方正品全系列',
      address: '尖沙咀廣東道 3-27 號海運大廈地下 OT G23 號舖',
      hours: '10:00 - 22:00',
      arrivalTime: '10:45 抵達寬敞舒適吹冷氣採購',
      transit: {
        line: '港鐵 荃灣線 / 屯馬線',
        station: '尖沙咀站 / 尖東站',
        exit: '尖沙咀站 A1 出口 / 尖東站 L6 出口',
        walk: '步行約 6 分鐘穿過廣東道直通海港城海運大廈'
      },
      tagText: '🔥 全港最大陀螺旗艦專櫃 & 官方對戰盤',
      tagColor: 'bg-purple-100 text-purple-800 border-purple-300',
      notes: '旗艦級規模！Beyblade X 發射器、手把、官方戰鬥盤、各代基本款盒裝庫存最足，不用擔心買到改裝假貨。'
    },
    {
      id: 'd4-3',
      day: 4,
      dayTitle: 'Day 4 (9/27 週日·中秋節)',
      timeSlot: '14:00 - 16:30',
      category: 'DRINK',
      highlightType: 'coffee',
      title: 'K11 MUSEA ＆ % ARABICA（海景咖啡）',
      subtitle: '全港最美藝術美學百貨 ＆ 維港海岸線精品咖啡',
      address: '尖沙咀梳士巴利道 18 號 K11 MUSEA 沿海亭（Victoria Dockside）',
      hours: '10:00 - 22:00',
      arrivalTime: '建議 14:00 午茶海景時段',
      transit: {
        line: '港鐵 屯馬線 / 荃灣線',
        station: '尖東站',
        exit: 'J 出口',
        walk: '尖東站 J 出口地下直通 K11 MUSEA'
      },
      tagText: '☕ 海景精品拿鐵 & 頂級藝術百貨',
      tagColor: 'bg-amber-100 text-amber-900 border-amber-300',
      notes: 'K11 MUSEA 建築極美、品牌齊全。戶外金方塊 % ARABICA 點杯 Spanish Latte 或熱美式，坐在維港邊吹海風極致享受。'
    },
    {
      id: 'd4-4',
      day: 4,
      dayTitle: 'Day 4 (9/27 週日·中秋節)',
      timeSlot: '17:00 - 18:30',
      category: 'FOOD',
      highlightType: 'burger',
      title: 'Shake Shack（海港城海景店） 或 Burger Circus',
      subtitle: '坐擁無敵維港海景的美式安格斯牛肉漢堡',
      address: '尖沙咀廣東道海港城海運大廈地下 OT G53 號舖',
      hours: '11:00 - 22:00',
      arrivalTime: '建議 17:00 入座看夕陽海景',
      transit: {
        line: '步行自維港海濱',
        station: '尖沙咀站',
        exit: '海港城海運大廈末端',
        walk: '就在天星碼頭旁海港城 3 樓露台 / 地下'
      },
      tagText: '🍔 海景美式厚切安格斯牛肉堡',
      tagColor: 'bg-emerald-100 text-emerald-800 border-emerald-300',
      notes: 'ShackBurger 肉餅鮮嫩多汁，起司波浪薯條香脆濃郁，戶外座位可一覽維港夕陽暮色。'
    },
    {
      id: 'd4-5',
      day: 4,
      dayTitle: 'Day 4 (9/27 週日·中秋節)',
      timeSlot: '19:00 - 20:30',
      category: 'SCENIC',
      highlightType: 'scenic',
      title: '天星小輪海景巡航 ＆ 維港中秋夜色',
      subtitle: '國家地理雜誌推薦人生必遊：百年渡海小輪',
      address: '尖沙咀天星碼頭（往中環/灣仔）',
      hours: '06:30 - 23:30',
      arrivalTime: '建議 19:00 搭乘（中秋滿月中維港海風）',
      transit: {
        line: '天星小輪渡輪（八達通感應進站）',
        station: '尖沙咀天星碼頭',
        exit: '刷卡即上船（上層景觀最佳約 HK$5.0）',
        walk: '海港城出門即達碼頭'
      },
      tagText: '🚢 維港中秋夜景渡輪',
      tagColor: 'bg-blue-100 text-blue-800 border-blue-300',
      notes: '只需銅板價就能從海中央仰望兩岸摩天大樓中秋燈火。💡【Threads 清單必吃 🍦】尖沙咀天星碼頭與鐘樓下常駐「富豪雪糕車」，上船前買一支香滑軟雪糕（HK$13）邊吹海風邊吃，氛圍感極佳！'
    },

    // --- DAY 5 ---
    {
      id: 'd5-1',
      day: 5,
      dayTitle: 'Day 5 (9/28 週一)',
      timeSlot: '09:00 - 10:15',
      category: 'DRINK',
      highlightType: 'tea',
      title: '紅茶冰室 Hung Tea Cafe（油麻地店）',
      subtitle: '油麻地最高人氣現代冰室 ＆ 厚茶底低糖絲滑熱奶茶',
      address: '油麻地彌敦道 498-500 號泰盛大廈地舖（近文明里）',
      hours: '07:00 - 23:00',
      arrivalTime: '建議 09:00 早餐時段',
      transit: {
        line: '港鐵 荃灣線 / 觀塘線',
        station: '油麻地站',
        exit: 'C 出口',
        walk: '出站正對面即達（過文明里馬路 10 秒）'
      },
      tagText: '☕ 沙爹牛肉麵 & 低糖絲滑奶茶',
      tagColor: 'bg-amber-100 text-amber-900 border-amber-300',
      notes: '在地人排隊名店！「沙爹牛肉公仔麵」濃郁微辛、「新鮮出爐脆皮蛋撻」奶香酥脆；熱奶茶點「少甜」，入口順滑厚實回甘。'
    },
    {
      id: 'd5-2',
      day: 5,
      dayTitle: 'Day 5 (9/28 週一)',
      timeSlot: '10:30 - 12:30',
      category: 'TOY',
      highlightType: 'toy',
      title: '油麻地／尖沙咀 伴手禮採買 ＆ 最後補陀螺',
      subtitle: '帝苑蝴蝶酥 / 小熊曲奇 ＆ 現時點最後補漏',
      address: '現時點（油麻地） / 珍妮曲奇（尖沙咀美麗都大廈 1 樓）',
      hours: '10:00 - 19:00',
      arrivalTime: '建議 10:30 起步採買',
      transit: {
        line: '港鐵 荃灣線',
        station: '油麻地站 / 尖沙咀站',
        exit: '油麻地 A2 / 尖沙咀 N5 出口',
        walk: '油麻地步行採買完後回飯店退房'
      },
      tagText: '🎁 伴手禮採買 & 玩具最後檢查',
      tagColor: 'bg-slate-100 text-slate-800 border-slate-300',
      notes: '採購經典蝴蝶酥與餅乾；若陀螺有少買的零件，可在 11:30 後於油麻地/旺角周邊模型店進行最後補貨。'
    },
    {
      id: 'd5-3',
      day: 5,
      dayTitle: 'Day 5 (9/28 週一)',
      timeSlot: '13:00 ➔ 前往機場',
      category: 'SCENIC',
      highlightType: 'scenic',
      title: '機場快線九龍站 ➔ 香港國際機場',
      subtitle: '免搭的士，22 分鐘直達機場航廈',
      address: '港鐵九龍站機場快線月台',
      hours: '05:53 - 00:48',
      arrivalTime: '依班機起飛前 2.5～3 小時抵達機場',
      transit: {
        line: '機場快線 (Airport Express)',
        station: '九龍站',
        exit: '直達機場客運大樓月台',
        walk: '油麻地搭乘公車/港鐵至九龍站轉乘機場快線（或預約市區預辦登機）'
      },
      tagText: '✈️ 機場快線快速返台',
      tagColor: 'bg-blue-100 text-blue-800 border-blue-300',
      notes: '全程平穩寬敞無塞車風險，車廂附有免費充電插座與行李架，輕鬆結束 5 天充實旅程。'
    }
  ];

  // 篩選邏輯
  const filteredList = itineraryData.filter(item => {
    const matchDay = activeDay === 0 || item.day === activeDay;
    const matchTag = selectedTag === 'ALL' || item.category === selectedTag;
    return matchDay && matchTag;
  });

  return (
    <div className="min-h-screen bg-slate-100 text-slate-900 pb-16 font-sans antialiased">
      {/* 頂部導航列與標題 (移除 sticky 以防手機版遮擋大部分視野) */}
      <header className="bg-gradient-to-r from-slate-900 via-indigo-950 to-slate-900 text-white py-5 sm:py-7 px-4 sm:px-6 shadow-md relative">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2 text-amber-400 text-xs sm:text-sm font-semibold tracking-wider uppercase mb-1">
                <span>🇭🇰 香港 5 天 4 夜深度自由行</span>
                <span className="bg-amber-400/20 text-amber-300 px-2 py-0.5 rounded-full text-xs">2026/09/24 - 09/28</span>
              </div>
              <h1 className="text-xl sm:text-3xl font-extrabold tracking-tight">
                美食饗宴 ＆ 戰鬥陀螺·動漫玩具探索指南
              </h1>
              <p className="text-slate-300 text-xs sm:text-sm mt-1.5 flex items-center gap-2">
                <MapPin className="w-4 h-4 text-rose-400 shrink-0" />
                <span>住宿基地：<strong>油麻地</strong> ｜ 港鐵、廣深港高鐵、電車、渡輪 ｜ <strong>超高效率無痛銜接</strong></span>
              </p>
            </div>

            <button
              onClick={() => setShowTips(!showTips)}
              className="bg-indigo-600/80 hover:bg-indigo-600 text-white px-3.5 py-2 rounded-xl text-xs sm:text-sm font-medium transition flex items-center gap-2 shadow-sm backdrop-blur-sm border border-indigo-400/30 cursor-pointer"
            >
              <Info className="w-4 h-4 text-amber-300" />
              <span>點餐・通關與陀螺攻略小抄</span>
            </button>
          </div>

          {/* 喜好類別快速篩選 */}
          <div className="mt-4 sm:mt-6 flex flex-wrap gap-2 pt-2 border-t border-slate-700/60">
            {tagDefinitions.map(t => {
              const Icon = t.icon;
              const isActive = selectedTag === t.id;
              return (
                <button
                  key={t.id}
                  onClick={() => setSelectedTag(t.id)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold transition shadow-sm cursor-pointer ${
                    isActive ? t.color : 'bg-slate-800/90 text-slate-300 hover:bg-slate-700'
                  }`}
                >
                  <Icon className="w-3.5 h-3.5" />
                  <span>{t.label}</span>
                </button>
              );
            })}
          </div>
        </div>
      </header>

      {/* 攻略提示摺疊區塊 */}
      {showTips && (
        <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-6">
          <div className="bg-amber-50 border border-amber-200 rounded-2xl p-5 shadow-sm text-slate-800 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="space-y-1">
              <h4 className="font-bold text-amber-900 text-sm flex items-center gap-1.5">
                <Coffee className="w-4 h-4 text-amber-700" /> 奶茶點單秘訣（不甜又絲滑）
              </h4>
              <p className="text-xs text-amber-800 leading-relaxed">
                港式奶茶預設加糖較甜。點單請務必說：<strong>「熱奶茶，少甜」</strong>或<strong>「熱奶茶，走甜（無糖）」</strong>，由店家附糖包自己微量調整，能喝到最極致的黑白淡奶厚實乳香與回甘茶感。
              </p>
            </div>
            <div className="space-y-1">
              <h4 className="font-bold text-purple-900 text-sm flex items-center gap-1.5">
                <ShoppingBag className="w-4 h-4 text-purple-700" /> 陀螺 & 吉伊卡哇採購指引
              </h4>
              <p className="text-xs text-purple-800 leading-relaxed">
                <strong>現時點（In’s Point 2F）</strong>店家約 16:30 陸續開齊，陀螺抽包/單售散件最便宜；<strong>朗豪坊 10-11F</strong> 專攻 Chiikawa 盲盒；<strong>海港城玩具"反"斗城</strong> 買盒裝對戰盤最完整。
              </p>
            </div>
            <div className="space-y-1">
              <h4 className="font-bold text-rose-900 text-sm flex items-center gap-1.5">
                <Utensils className="w-4 h-4 text-rose-700" /> 蓮香樓推車飲茶 ＆ 蘭桂坊
              </h4>
              <p className="text-xs text-rose-800 leading-relaxed">
                <strong>蓮香樓</strong>保留百年手推點心車，看到阿姐推車即可拿點心卡前往蓋章拿取（必點豬潤燒賣與霸王鴨）；<strong>蘭桂坊</strong>白天悠閒好拍歐式街景，入夜後則化身熱鬧的特色餐酒吧街。
              </p>
            </div>
            <div className="space-y-1">
              <h4 className="font-bold text-emerald-900 text-sm flex items-center gap-1.5">
                <Train className="w-4 h-4 text-emerald-700" /> 交通與深圳通關指南
              </h4>
              <p className="text-xs text-emerald-800 leading-relaxed">
                香港全程以 Apple/Google <strong>八達通</strong> 嗶卡。Day 2 去深圳帶<strong>台胞證</strong>刷閘門，高鐵 14 分鐘直達福田；微信/支付寶綁台灣信用卡支付；<strong>霸王茶姬與瑞幸</strong>建議提早以小程式遠端下單秒取杯；洗頭請交待「<strong>用指腹輕揉放鬆</strong>」。
              </p>
            </div>
          </div>
        </div>
      )}

      {/* 天數選擇 Tabs */}
      <div className="max-w-6xl mx-auto px-4 sm:px-6 pt-6">
        <div className="bg-white p-1.5 rounded-2xl shadow-sm border border-slate-200/80 flex overflow-x-auto no-scrollbar gap-1">
          {[
            { id: 0, label: '全部行程', sub: '5 天彙整' },
            { id: 1, label: 'Day 1 (9/24)', sub: '放行李/越式Pho/陀螺/一點心' },
            { id: 2, label: 'Day 2 (9/25)', sub: '高鐵深圳/牛肉鍋/COCO Park/茶姬瑞幸/盒馬' },
            { id: 3, label: 'Day 3 (9/26)', sub: '大館/蓮香樓/蘭桂坊/山頂纜車' },
            { id: 4, label: 'Day 4 (9/27)', sub: '滑蛋/海景咖啡/渡輪' },
            { id: 5, label: 'Day 5 (9/28)', sub: '紅茶冰室/伴手禮/返台' },
          ].map(tab => (
            <button
              key={tab.id}
              onClick={() => setActiveDay(tab.id)}
              className={`flex-1 min-w-[120px] py-2.5 px-3 rounded-xl text-center transition cursor-pointer ${
                activeDay === tab.id
                  ? 'bg-slate-900 text-white shadow font-semibold'
                  : 'text-slate-600 hover:bg-slate-100'
              }`}
            >
              <div className="text-xs sm:text-sm font-bold leading-tight">{tab.label}</div>
              <div className={`text-[10px] mt-0.5 ${activeDay === tab.id ? 'text-amber-300' : 'text-slate-400'}`}>
                {tab.sub}
              </div>
            </button>
          ))}
        </div>
      </div>

      {/* 卡片主展示區 */}
      <main className="max-w-6xl mx-auto px-4 sm:px-6 pt-6">
        {filteredList.length === 0 ? (
          <div className="bg-white rounded-2xl p-12 text-center text-slate-500 border border-slate-200">
            <Filter className="w-10 h-10 mx-auto text-slate-300 mb-2" />
            <p className="text-base font-medium">此篩選條件下無符合項目</p>
            <button 
              onClick={() => { setActiveDay(0); setSelectedTag('ALL'); }}
              className="mt-3 text-xs text-indigo-600 font-semibold underline cursor-pointer"
            >
              重設所有篩選
            </button>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredList.map((item) => {
              // 特殊醒目框樣式判斷
              const isToy = item.highlightType === 'toy';
              const isDrink = item.highlightType === 'tea' || item.highlightType === 'coffee';
              const isFood = item.highlightType === 'burger' || item.highlightType === 'pho';

              let borderAccent = 'border-slate-200 hover:border-slate-300';
              let topGlow = '';
              if (isToy) {
                borderAccent = 'border-purple-300 hover:border-purple-500 ring-1 ring-purple-200/70';
                topGlow = 'bg-gradient-to-r from-purple-500 to-indigo-500';
              } else if (isDrink) {
                borderAccent = 'border-amber-300 hover:border-amber-500 ring-1 ring-amber-200/70';
                topGlow = 'bg-gradient-to-r from-amber-500 to-orange-500';
              } else if (isFood) {
                borderAccent = 'border-emerald-300 hover:border-emerald-500 ring-1 ring-emerald-200/70';
                topGlow = 'bg-gradient-to-r from-emerald-500 to-teal-500';
              }

              return (
                <div
                  key={item.id}
                  className={`bg-white rounded-2xl shadow-sm hover:shadow-md transition-all duration-200 flex flex-col justify-between overflow-hidden border ${borderAccent}`}
                >
                  {/* 頂部主題彩色微條 */}
                  {topGlow && <div className={`h-1.5 w-full ${topGlow}`} />}

                  <div className="p-5 flex-1 flex flex-col justify-between">
                    <div>
                      {/* 時段與標籤 */}
                      <div className="flex items-center justify-between gap-2 mb-2.5">
                        <span className="inline-flex items-center gap-1 text-[11px] font-bold text-slate-500 bg-slate-100 px-2.5 py-1 rounded-md">
                          <Clock className="w-3 h-3 text-slate-400" />
                          {item.timeSlot}
                        </span>
                        <span className={`text-[11px] font-bold px-2.5 py-1 rounded-md border ${item.tagColor}`}>
                          {item.tagText}
                        </span>
                      </div>

                      {/* 標題與簡介 */}
                      <h3 className="text-lg font-extrabold text-slate-900 leading-snug tracking-tight">
                        {item.title}
                      </h3>
                      <p className="text-xs text-slate-500 mt-0.5 font-medium">
                        {item.subtitle}
                      </p>

                      {/* 重點營業與抵達時間欄位 */}
                      <div className="mt-3.5 space-y-2 bg-slate-50/80 p-3 rounded-xl border border-slate-100 text-xs">
                        <div className="flex items-start gap-2 text-slate-700">
                          <Clock className="w-3.5 h-3.5 text-indigo-500 mt-0.5 shrink-0" />
                          <div>
                            <span className="font-semibold text-slate-900">營業時間：</span>
                            <span>{item.hours}</span>
                          </div>
                        </div>

                        <div className="flex items-start gap-2 text-amber-900 bg-amber-50/80 p-1.5 rounded-lg border border-amber-200/50">
                          <Bookmark className="w-3.5 h-3.5 text-amber-600 mt-0.5 shrink-0" />
                          <div>
                            <span className="font-bold">建議時段：</span>
                            <span>{item.arrivalTime}</span>
                          </div>
                        </div>
                      </div>

                      {/* 醒目的地鐵交通框（格子框化） */}
                      <div className="mt-3 bg-gradient-to-br from-indigo-50/70 to-slate-50 p-3 rounded-xl border border-indigo-100 text-xs">
                        <div className="flex items-center gap-1.5 font-bold text-indigo-950 mb-1">
                          <Train className="w-3.5 h-3.5 text-indigo-600" />
                          <span>港鐵交通指引</span>
                        </div>
                        <div className="space-y-1 text-slate-700">
                          <div className="flex items-center gap-1.5">
                            <span className="font-semibold text-indigo-900 bg-indigo-100/70 px-1.5 py-0.5 rounded text-[11px]">
                              {item.transit.line}
                            </span>
                            <span className="font-bold text-slate-900">{item.transit.station}</span>
                            <span className="bg-rose-50 text-rose-700 font-extrabold px-1.5 py-0.2 rounded border border-rose-200 text-[11px]">
                              {item.transit.exit}
                            </span>
                          </div>
                          <p className="text-[11px] text-slate-600 pl-0.5 leading-tight pt-0.5">
                            📍 {item.transit.walk}
                          </p>
                        </div>
                      </div>

                      {/* 地址欄與一鍵 Google Maps 導航 / 複製 */}
                      <div className="mt-3 bg-slate-50/90 rounded-xl p-2.5 border border-slate-200/80 space-y-2">
                        <div className="flex items-start gap-1.5 text-xs text-slate-600">
                          <MapPin className="w-3.5 h-3.5 text-rose-500 mt-0.5 shrink-0" />
                          <span className="leading-tight text-[11px] font-medium text-slate-700">{item.address}</span>
                        </div>
                        
                        <div className="flex items-center gap-2 pt-1 border-t border-slate-200/60">
                          {/* 一鍵 Google Maps 搜尋導航按鈕 */}
                          <a
                            href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(getMapQuery(item.title, item.address))}`}
                            target="_blank"
                            rel="noopener noreferrer"
                            title="一鍵在 Google Maps 開啟並搜尋導航"
                            className="flex-1 inline-flex items-center justify-center gap-1.5 py-1.5 px-3 bg-rose-500 hover:bg-rose-600 active:scale-[0.98] text-white text-xs font-semibold rounded-lg shadow-xs transition cursor-pointer"
                          >
                            <Navigation className="w-3.5 h-3.5 text-rose-100" />
                            <span>Google Maps 導航</span>
                          </a>

                          {/* 一鍵複製地址按鈕 */}
                          <button
                            onClick={() => handleCopy(item.address, item.id)}
                            title="複製完整地址"
                            className={`inline-flex items-center justify-center gap-1 py-1.5 px-2.5 text-xs font-medium rounded-lg border transition cursor-pointer shrink-0 ${
                              copiedId === item.id
                                ? 'bg-emerald-50 text-emerald-700 border-emerald-300'
                                : 'bg-white hover:bg-slate-100 text-slate-600 border-slate-200'
                            }`}
                          >
                            {copiedId === item.id ? (
                              <>
                                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                                <span className="text-[11px] font-bold">已複製</span>
                              </>
                            ) : (
                              <>
                                <Copy className="w-3.5 h-3.5 text-slate-400" />
                                <span className="text-[11px]">複製地址</span>
                              </>
                            )}
                          </button>
                        </div>
                      </div>
                    </div>

                    {/* 底部老饕與採購心得提示 */}
                    <div className="mt-3.5 pt-3 border-t border-dashed border-slate-200">
                      <p className="text-xs text-slate-600 leading-relaxed">
                        <strong className="text-slate-800">💡 重點備忘：</strong>{item.notes}
                      </p>
                    </div>
                  </div>
                </div>
              );
            })}
          </div>
        )}
      </main>

      {/* 底部總覽小結 */}
      <footer className="max-w-6xl mx-auto px-4 sm:px-6 mt-12 text-center text-xs text-slate-500">
        <p>港鐵營運時間約為 06:00 至 00:30 ｜ 各店家營業時間請以現場實際情況為準</p>
        <p className="mt-1 font-medium text-slate-600">祝您在香港擁有豐富滿足的美食與動漫玩具之旅！🇭🇰✨</p>
      </footer>
    </div>
  );
}
